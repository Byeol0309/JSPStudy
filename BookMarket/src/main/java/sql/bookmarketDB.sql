...
USE BookMarketDB; --현재 사용자가 정의된 데이터베이스를 쓰겠다

CREATE TABLE IF NOT EXISTS book( -- IF NOT EXISTS book book table이 없으면 create
...
CREATE TABLE board (
       num int not null auto_increment,		-- 게시글순번
       ...
       hit int,								-- 게시글 조회수
       ip varchar(20),						-- 게시글 등록시 IP
       PRIMARY KEY (num)					-- 게시글 순번을 고유키로 설정
)default CHARSET=utf8;
